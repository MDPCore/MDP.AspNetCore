...
{
    public class TokenAuthenticationFactory : ServiceFactory<WebApplicationBuilder, TokenAuthenticationFactory.SettingDictionary>
    {
        // Constructors
        public TokenAuthenticationFactory() : base("Authentication", "Tokens", false) { }


        // Methods
        public override void ConfigureService(WebApplicationBuilder applicationBuilder, TokenAuthenticationFactory.SettingDictionary settingDictionary)
        {
            #region Contracts
            ...
            foreach (var setting in settingDictionary)
            {
                ...
                // TokenAuthentication
                authenticationBuilder.AddTokenAuthentication(setting.Key, setting.Value.Header, setting.Value.Prefix, setting.Value.Algorithm, setting.Value.SignKey, setting.Value.Issuer);

                // TokenAuthenticationSelector
                authenticationBuilder.AddTokenAuthenticationSelector(setting.Key, setting.Value.Header, setting.Value.Prefix);
            }
        }
