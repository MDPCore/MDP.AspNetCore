...
專案開發過程，需要提供Google身分驗證，讓使用者能夠快速登入系統。本篇範例協助開發人員使用MDP.AspNetCore.Authentication.Google，逐步完成必要的設計和實作。
...
9.於Google身分驗證服務完成身分驗證之後，Browser視窗會跳轉回原系統的Home頁面，並且顯示目前User的身分資料。(經由Google身分驗證登入)
